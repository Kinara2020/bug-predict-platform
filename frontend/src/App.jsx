...
export default function App() {
  ...
  const runScan = async () => {
    if (!owner || !repo) return setError("Enter both owner and repo");
    setError("");
    setLoading(true);
    setResult(null);
    try {
        ...
        <>
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={chartData}>
                ...
                <Bar dataKey="bug_risk_score" radius={[6, 6, 0, 0]}>
                  {chartData?.map((f, i) => <Cell key={i} fill={riskColor(f.bug_risk_label)} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </section>
              ...
              <div className="issue-list">
                {result.security_issues.map((s, i) => (
                  <div key={i} className={`issue-row sev-${s.severity.toLowerCase()}`}>
                    ...
                    <span className="issue-path">{s.file_path}:{s.line}</span>
          ...
          <section className="panel">
            ...
            <div className="risk-grid">
              {result.file_metrics.filter(f => f.bug_risk_label !== "low").map((f, i) => (
